...
def main():
    ...
    channels_lookup = {}
    for hemisphere, channels in settings["chb-mit"]["univariate_channels_groups"].items():
                ...
                for band_name, band in zip(["delta", "theta", "alpha", "beta", "gamma", "all"],
                    ...
                    for channel_number, channel_name in enumerate(metadata["channels"]):
                        if (metadata["seizure_type"] != "bilateral" and
                            channel_name not in settings["chb-mit"]["univariate_channels_groups"][metadata["seizure_type"]]):
                            continue
